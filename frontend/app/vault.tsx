...
import { api, apiText, apiBlob, COLORS, useAuth, canViewVault, isFinanceAdmin } from '../src/lib/auth';
...
const recentWednesdays = (n: number): string[] => {
  const out: string[] = [];
  const d = new Date();
...
export default function VaultScreen() {
   ...
   *  owner, 2026-09-17): Top 4 Producers, Top Rookie Producers, Top Plus Lead
   *  Sales, Top Plus Leads Collected. Every office, agents and leaders
   *  together, one Wed-to-Tue week. Built from live production data, so any
   *  recent week works — not just the archived snapshots below.
   *  Web only, like the WAR workbooks: a binary file needs a real download. */
  const [sauceWeek, setSauceWeek] = useState<string>(() => recentWednesdays(8)[0]);
  const [exportingSauce, setExportingSauce] = useState(false);
  const exportSecretSauce = async () => {
    if (Platform.OS !== 'web' || typeof document === 'undefined') {
      notify('Open on the web', 'Morgans Secret Sauce downloads from the web app — the phone can only share text.');
      return;
    }
    setExportingSauce(true);
    try {
      const blob = await apiBlob(`/api/vault/secret-sauce?week_start=${sauceWeek}`);
      const href = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = href; link.download = `${sauceWeek}_Morgans_Secret_Sauce.xlsx`; link.click();
      URL.revokeObjectURL(href);
    } catch (e: unknown) {
      notify('Export failed', e instanceof Error ? e.message : 'Could not build the report.');
    } finally {
      setExportingSauce(false);
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        {/* Morgans Secret Sauce — weekly top-producer sheet, any recent week. */}
        <View style={styles.sauceCard} testID="vault-secret-sauce">
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>
            {recentWednesdays(8).map((w) => {
              const sel = w === sauceWeek;
              return (
                <TouchableOpacity
                  key={w}
                  onPress={() => setSauceWeek(w)}
                  style={[styles.tab, sel && styles.tabOn]}
                  testID={`vault-sauce-week-${w}`}
                >
          ...
          <TouchableOpacity
            onPress={exportSecretSauce}
            disabled={exportingSauce}
            style={styles.sauceBtn}
            testID="vault-export-secret-sauce"
          >
            <Ionicons name="flame" size={14} color="#000" />
            <Text style={styles.sauceBtnTxt}>{exportingSauce ? 'BUILDING…' : 'MORGANS SECRET SAUCE'}</Text>
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  sauceCard: { backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, borderTopWidth: 2, borderTopColor: COLORS.gold, borderRadius: 6, padding: 14, marginTop: 20 },
  sauceBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 12, backgroundColor: COLORS.gold, borderRadius: 6, paddingVertical: 10 },
  sauceBtnTxt: { color: '#000', fontSize: 11, fontWeight: '900', letterSpacing: 1.4 },
